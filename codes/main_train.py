...
from src.utility.data import read_data, CreateDataset
from src.utility.plot_results import loss_and_accuracy, performance
from src.model.model import MFCCModel
from src.model.model_utils import train_model, evaluate_model
...
def main():
    ...
    output_path = conf['paths']['kfold_output_path']
    fold_dest = conf['paths']['kfold_fold_path']
    ...
    batch_size = conf['train_config']['batch_size']
    epochs = conf['train_config']['max_epochs']
    learning_rate = conf['train_config']['learning_rate']
    weight_decay = conf['train_config']['weight_decay']
    n_folds = conf['train_config']['kfold']
    dropout = conf['train_config']['dropout']
    positive_weight = torch.tensor([conf['train_config']['positive_weight']], dtype=torch.float).to(device)
    model_reduction = conf['train_config']['reduction']
